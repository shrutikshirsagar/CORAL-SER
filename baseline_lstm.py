...
def main():
    ...
    shift_sec     = 0.3  # Shift of annotations for training (in seconds)
   ...
   # pred_test_DE = []  # Store (best) test predictions
    ...
    while iteration <= num_iter:
        ...
        #print(train_x.shape)
        #print(train_y.shape)
        model.fit(train_x, train_y, batch_size=batch_size, epochs=500)  # Evaluate after each epoch
         ...
         #   seq_len    = pred_inst.shape[0]
          #  time_stamp = np.linspace(0., (seq_len-1)/float(inst_per_sec), seq_len).reshape(-1,1)
           # pred_inst  = np.concatenate( (time_stamp, pred_inst), axis=1 )
            ...
            #save_features(filename, pred_inst, append=True, instname=instname, precision=6)
